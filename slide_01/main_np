...
def get_distance(fg, bg):
    ...
    br = (tl[0] + tw, tl[1] + th)
    im = cv2.rectangle(template, tl, br, (0, 0, 255), 2)
...
if __name__=='__main__':
    ...
    #读取文件
        imageContent1 = BytesIO(f1.read())
    with open(image2, "rb") as f1:
    #读取文件
        imageContent2 = BytesIO(f1.read())

    x = get_distance(imageContent1, imageContent2)
    print(x)
    print("hello world")
